...
**Packages**:
- `@trpc/server` v`^11.8.1` (see `package.json:59`)
- `@trpc/client` v`^11.8.1` (see `package.json:58`)
- `@trpc/tanstack-react-query` v`^11.8.1` (see `package.json:60`)

**Role**: End-to-end type safety between client and server.

**Architecture** (see `src/trpc/routers/_app.ts:7`):
```typescript
appRouter = {
  usage: usageRouter,       // src/modules/usage/server/procedures.ts
  messages: messagesRouter, // src/modules/messages/server/procedures.ts
  projects: projectsRouter  // src/modules/projects/server/procedures.ts
}
```
...
**Role**: Schema validation for APIs and forms.
