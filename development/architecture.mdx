...
- **Convex** as a real-time database with reactive queries
...
│                     Application Layer                       │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │  App Router  │  │  API Routes  │  │ tRPC Routers │     │
│  │   (Pages)    │  │ (/api/*)     │  │  (Modules)   │     │
│  └──────────────┘  └──────────────┘  └──────────────┘     │
└─────────────────┬───────────────────────────────────────────┘
                  │
...
### 5. Type-Safe API Layer (tRPC)

**Location**: `src/trpc/`

tRPC provides end-to-end type safety between client and server:

**Router Structure** (see `src/trpc/routers/_app.ts:7`):
```typescript
appRouter {
  usage: usageRouter,      // Credit tracking, rate limits
  messages: messagesRouter, // Message CRUD, streaming
